...
void helmholtz ( int m, int n, int it_max, double alpha, double omega, double tol ) {
  ...
  #pragma omp parallel for private(j,i) shared(u)
  for ( j = 0; j < n; j++ )
  {
...
void error_check ( int m, int n, double alpha, double u[], double f[] ) {
  ...
  #pragma omp parallel for private(j,i) reduction(+:u_norm)
  for ( j = 0; j < n; j++ )
  {
    #pragma omp simd aligned(u:64)
    for ( i = 0; i < m; i++ )
  ...
  #pragma omp parallel for private(j,i) reduction(+:error_norm, u_true_norm) 
  for ( j = 0; j < n; j++ )
  {
    for ( i = 0; i < m; i++ )
    {
...
*/
  ...
  for ( it = 1; it <= it_max; it++ )
  {
    ...
    #pragma omp parallel for private(j,i) reduction(+:error_norm)
    for ( j = 0; j < n; j++ )
    {
      #pragma omp simd aligned(u,u_old:64)
      for ( i = 0; i < m; i++ )
...
double *rhs_set ( int m, int n, double alpha ) {
  ...
  f = ( double * ) _mm_malloc ( m * n * sizeof ( double ),64 );
    ...
    #pragma omp parallel for simd aligned(f:64)
    for ( i = 0; i < m; i++ )
    {
      j = 0;
      y = ( double ) ( 2 * j - n + 1 ) / ( double ) ( n - 1 );
  ...
  #pragma omp parallel for reduction(+:f_norm)
  for ( j = 0; j < n; j++ )
  {
    #pragma omp simd aligned(f:64)
    for ( i = 0; i < m; i++ )
